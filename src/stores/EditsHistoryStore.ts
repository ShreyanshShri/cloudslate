...
import { addEntity, updateEntity, deleteEntity, clearEntity } from "./utils/editHistoryStateFunctions";
import { addEntityToServer, updateEntityInServer, deleteEntityInServer } from './utils/editHistoryServerFunctions';

type editHistoryStore = {
    history: history_el[],
    addEntity: (entity: entity_data) => void,
    updateEntity: (entity: entity_data, index: number) => void,
...
const hardSave = async (file_id: string) => {
    ...
    const clearEntity = useEditHistoryStore.getState().clearEntity;
    try {
        const response = await axios.put(`${import.meta.env.VITE_SERVER_URL}/editor/edit/all?id=${file_id}`, {
            entities            
        }, {
            headers: {
...
const useEditHistoryStore = create<editHistoryStore>(zukeeper((set: any) => ({
    history: [],
    addEntity: (entity: entity_data) => addEntity(set, entity),
    updateEntity: (entity: entity_data, index: number) => updateEntity(set, entity, index),
