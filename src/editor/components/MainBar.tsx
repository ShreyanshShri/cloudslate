...
function MainBar() {
    ...
    const check_id_and_get_file = async () => {
        if(id == undefined) {
            try {
                const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}/editor/new`, {}, {
                    headers: {
        ...
        try {
            const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}/editor/get?id=${id}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`
                }
            });
        ...
        <div id="main-bar">
            ...
            {file.entities.map((en: any, index: number) => {
                ...
                if(en.type === "whiteboard") {
                    return (<Whiteboard key={index} index={index} snapshot={JSON.parse(en.data)} />)
                }
            })}
        </div>
    )
